...
@SpringBootTest(
        ...
        properties = {
            "spring.cloud.gateway.server.webflux.routes[0].id=users",
            ...
            "spring.cloud.gateway.server.webflux.routes[0].filters[0].args.key-resolver=#{@remoteAddressKeyResolver}"
        })
...
class RateLimiterTest {
    ...
    @Test
    void testRateLimiter_blocksRequestsOverLimit() {
        ...
        await().pollInterval(100, MILLISECONDS).atMost(2, SECONDS).untilAsserted(() -> {
            webTestClient
                    .get()
                    .uri("/api/users")
                    .header("X-Forwarded-For", "192.168.1.1")
        ...
        await().timeout(2, SECONDS).pollDelay(1, SECONDS).untilAsserted(() -> Assertions.assertTrue(true));
